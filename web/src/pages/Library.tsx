...
export function Library() {
  ...
  const [pane, setPane] = useState<'projects' | 'developers'>('projects');
    ...
    <>
      ...
      >
        <span className="seg" style={{ margin: 0 }}>
          <button type="button" className={pane === 'projects' ? 'on' : ''} onClick={() => setPane('projects')}>
            Projects
          </button>
          <button type="button" className={pane === 'developers' ? 'on' : ''} onClick={() => setPane('developers')}>
            Developers
          </button>
        </span>
